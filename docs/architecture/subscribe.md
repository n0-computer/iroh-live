...
In iroh-live, `Live::subscribe` builds the `RemoteBroadcast` and wraps it in a
`Subscription`, which also attaches the connection's link signals so every
player of `sub.broadcast()` adapts. See [adaptive bitrate](adaptive.md).

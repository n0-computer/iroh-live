...
Depends on: the pre-encoded-source concept buy-in (coordination point 5); independent of the base API contract
Path: independent
Size: M
...
path on top of it. This is moq-changes change 12. It has two parts, in order:
first a concept proposal, a `publish_preencoded` sibling of `publish_capture`
that takes a source yielding H.264 (or H.265) access units plus a hang catalog
config and drives them straight to the track, and second the concrete
implementation, a libcamera backend that runs `rpicam-vid`'s hardware H.264
encoder and emits Annex-B directly. The concept is the load-bearing part and must
be agreed with the maintainer before implementation, because it asks moq to
accept that a source may bypass the encoder, which no existing moq path does.
...
(`comparisons/capture.md:487-492, 523-525`).
...
## The concept proposal (present first, implement second)
...
1. Is a source that bypasses the encoder acceptable in moq's model at all, given
   ...
   machinery exists; the ask is only the turnkey source-side loop and the
   acceptance of an encoder-bypassing source as a first-class concept.
...
Do not implement past this gate until the concept is agreed. If the maintainer
declines the bypass concept, the fallback is to keep the libcamera H.264 source
in iroh-live and consume moq's `Producer::publish` directly, which needs no moq
change, and the leaf closes as "declined, kept local."
...
- Coordination point 5 (the pre-encoded-source concept). This is the gate. The
  `publish_preencoded` concept plus the "a source may bypass the encoder"
  acceptance needs maintainer buy-in before any implementation past step 1's
  synthetic-source proof. Lead the conversation with the concept, not the Pi
  backend; the backend is the motivating user, the concept is the ask.
- Independent of the base API contract: this leaf touches no `Native`/`Frame`
  ...
  coordination points 1 and 2 do not apply. Its only gate is point 5.
- If declined, fall back to keeping `LibcameraH264Source` in iroh-live over
  moq's existing `Producer::publish`, needing no moq change.

## Acceptance checklist

- The concept is agreed with the maintainer before implementation past the
  synthetic-source proof (coordination point 5 satisfied and recorded in the PR).

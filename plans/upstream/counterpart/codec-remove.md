...
`moq_mux::codec` rather than its own 22,310-line codec crate. This plan is the
iroh-live pair side of the upstream campaign's codec leaves: each gated cut lands
on the paired iroh-live `up/<name>` branch matching the moq leaf it waits on (the
...
Two removal kinds run side by side. Adopt-theirs backends (openh264, VideoToolbox
encode, the bitstream front end, and the Candidate dispatch) are replaced by code
already on moq main and land with the release bump. Upstream-ours-then-remove
backends (VAAPI, V4L2, Android, and Opus) are deleted only after the matching moq
leaf from this campaign merges into a moq release, because deleting them before
their replacement exists would drop hardware decode on Intel and AMD Linux, ARM
SoCs, and Android, and would regress the decode-to-render zero-copy inputs. AV1
is the exception among the once-held backends: it is not upstreamed this series
(rav1e is too slow and the rav1d git-fork dependency too heavy to carry now;
`../0-overview.md`, Goal and scope), so its disposition is defer upstream, drop
local: the module is ripped out locally with no moq replacement, deleted
immediately with the independent deletions rather than gated on an upstream
contribution, and it can be re-added later if a use case needs it. This is the
largest pair-side removal in the upstream campaign, roughly 12,000 to 15,000 LOC
...
- `../comparisons/codecs.md` carries the per-backend capability comparison and
  the verdict table (section 10), with the top upstream candidates in priority
  order.
- `../comparisons/zerocopy.md` sections 2b and 5 establish the zero-copy rule
  (`../cut-plan.md` P2): the decode-to-render zero-copy path survives only if
  `render-adopt` re-homes the renderer onto moq's public handles before these
  decoders are cut.
- Upstream contributions are the leaf plans under `../codec/` and `../audio/`,
  and the base plans under `../base/`.
...
- `moq-video` VAAPI and V4L2 backends, and the registered or in-tree Android
  ...
  replacement this series (deferred upstream, `../0-overview.md`); its local
  backend is ripped out rather than swapped for a moq equivalent.
...
The PCM codec (`rusty-codecs/src/codec/pcm/`, 559 LOC) is kept and upstreamed:
PCM is a required capability, contributed through `../audio/pcm.md` as
`Codec::Pcm` in moq-audio together with the hang catalog PCM variant in the same
branch. The local codec stays until the moq release carrying `Codec::Pcm` is
pinned, at which point the `up/pcm` pair branch adopts moq-audio's PCM and
deletes the local module; until then, and in the event the offer stalls, the
local codec remains supported so the capability is never lost (codecs.md sec 6).

Explicitly not deleted by this task: `processing/scale.rs` (360) and
`processing/convert.rs` (598), which stay serving capture and render (cut-plan
...
4. Hold macOS entirely on the local stack until `vtb-mf-decode-surface` has
   ...
   flip macOS atomically on the `up/vtb-mf-decode-surface` pair branch: adopt the
   VideoToolbox encoder and the surface-retaining decoder together, and delete
   `codec/vtb/encoder.rs` and `codec/vtb/decoder.rs` in one platform-scoped,
   revertible commit, so the CVPixelBuffer-to-render path never breaks (zero-copy
   rule; zerocopy.md sec 2b).
5. Hold Linux non-NVIDIA entirely until the VAAPI and V4L2 upstream series have
   merged and released. Then flip Linux atomically across the `up/vaapi-decode`,
   `up/vaapi-encode`, `up/v4l2-encode`, and `up/v4l2-decode` pair branches: pin
   the moq release carrying the upstreamed VAAPI (encode and decode over the
   grown moq-vaapi) and V4L2 backends, and delete `codec/vaapi/` and
   `codec/v4l2/` together with the version bump in the same commit (cut-plan
   commit strategy: the deletion and the bump travel together). The
   DMA-BUF-to-render path must already run through `render-adopt` before this
   deletion.
6. Hold Android until `android-mediacodec` is accepted (in-tree or registered per
   the D2 decision), then delete `codec/android/` on the `up/android-mediacodec`
   pair branch, preserving the HardwareBuffer decode-to-render input via
   `render-adopt` (zerocopy.md sec 2b).
7. Delete `codec/av1/` as a local rip-out with the independent local deletions,
   not gated on any upstream contribution, since AV1 is deferred upstream and
   dropped locally this series. Software AV1 decodes to CPU I420 and feeds no
   zero-copy render input, so its removal regresses no held frame model and needs
   no platform-atomic hold. The proof-before-deletion rule still applies: a local
   end-to-end test must pass without the AV1 backend before the module is
   deleted. AV1 can be re-added later if a use case needs it.
...
Mandatory, per the proof-before-deletion rule (`../cut-plan.md` P1): no module is
deleted until the new path passes an end-to-end test in this repository.
...
- The proof-before-deletion rule (`../cut-plan.md` P1) governs every row:
  adoption behind a feature flag, both paths tested, default flipped, then a
  deletion-only commit.
- The zero-copy rule (`../cut-plan.md` P2): every decode deletion (VTB, VAAPI,
  Android) waits for `render-adopt` and the B1 frame vocabulary so the
...
- The AV1 module (`codec/av1/`) is ripped out locally with the independent
  deletions, not gated on an upstream release (deferred upstream, dropped local),
  after a local end-to-end test passes without it.
- Opus is deleted only after `opus-improvements` releases the three required
  items.
- The codec half of `traits.rs` and the frame-model half of `format.rs` are gone;
  the device traits, `scale.rs`, `convert.rs`, and PCM (until the `up/pcm`
  adoption) remain.

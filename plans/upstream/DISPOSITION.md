...
| Catalog config mirror | `rusty-codecs/src/config.rs` | 318 | adopt-theirs | `cut-plan.md` stage M0 | Replaced by direct `hang::catalog` types. The mirror no longer compiles against hang 0.19.5 (codecs.md, final section), so this cut is already forced. |
...
| SPS VUI patcher | `rusty-codecs/src/codec/h264/sps.rs` | 586 | upstream-ours | `codec/bitstream-sps-vui.md` | The patcher logic is offered upstream as an optional low-latency decode pass. The local file is dead code and is deleted without waiting (cut-plan rusty-codecs table); the leaf ports from git history. |
| VAAPI encode and decode | `rusty-codecs/src/codec/vaapi/` | 3,257 | upstream-ours | `codec/vaapi-encode.md`, `codec/vaapi-decode.md` | Zero-copy DMA-BUF import, VPP, PRIME export, and the only VAAPI decode on either side (codecs.md sec 1). |
...
| Opus wrapper | `rusty-codecs/src/codec/opus/` | 804 | adopt-theirs (after opus-improvements) | `audio/opus-improvements.md` | The wrapper shell is adopted from moq-audio; our control surface (runtime `set_bitrate`, lookahead pre-skip, FEC/DTX ctl plumbing, decoder remix) merges upstream first. The local module is deleted after the merge releases. |
...
| Scale and colorspace convert | `rusty-codecs/src/processing/scale.rs`, `convert.rs` | 958 | keep | n/a | Serve capture and render locally; moq leaves scaling to the caller and already uses the `yuv` crate. Shrinks as its consumers migrate; no moq counterpart to converge on. |
...
| X11 MIT-SHM screen | `rusty-capture/src/platform/linux/x11.rs` | 373 | upstream-ours (leaf pending) | `cut-plan.md` rusty-capture table | Portal-less X11 screen capture, which moq lacks entirely (`Error::Unsupported`, capture.md sec 2). Capture-land and missing in moq, so it upstreams; needs a small follow-up leaf after the Linux capture series. |

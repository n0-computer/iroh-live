...
The single structural consequence of the merge is that the three round-1 cut
scenarios collapse to two. Round 1 modeled (i) a thin main-only world of about
1,300 LOC, (ii) the dev stack shipped as-is at about 4,800 LOC, and (iii) dev
plus our accepted upstreams at about 17,400 LOC. World (i) no longer exists,
because the thin main it described has been superseded by the merged stack.
What remains is (A) adopt current moq main as-is, once iroh-live bumps to a moq
release that includes the merged stack, and (B) adopt plus land our upstream
contributions. Scenario A is the old (ii) and Scenario B is the old (iii); the
old (i) local wins are a subset of Scenario A rather than a standalone world.
...
**P3. One release gate, labeled on every item.** With the merge there is a
single moq codebase, so the round-1 "main-compatible versus dev-dependent"
split is gone. Two kinds of cut remain. Local cuts need no moq change and can
...
| codec/av1/ | 936 | keep-and-upstream-copy | upstream rav1e encode and rav1d decode as moq's software AV1 | rav1d git-fork pin resolved; D2(a); release | 3-compare sec 3; 3u sec 4 |
...
| codec/pcm/ | 559 | keep | none; optionally offer `Codec::Pcm` upstream | none | 3-compare sec 6; 6-compare 1.4 |
...
| render.rs + render/ | 3,463 | keep, then upstream as out-of-tree crate | no in-tree renderer exists upstream; becomes a published `moq-video-render` crate over U1 public handles (U4, Option B) | U1 before any upstreaming | 3z sec 4, sec 5 U4; 3u sec 1b |
...
| audio_backend.rs + aec.rs | 2,837 | keep | no moq equivalent (playback, mixing, fades, AEC, recovery); adopt their capture surface (system audio, TCC, `format()` without open) onto our bounded buffers | n/a | 6-compare sec 3, 7 |
...
final section; 4-compare section 5). The old round-1 local wins (dead code, the
broken config mirror, the transport handshake, the room announce migration) are
folded into this column rather than standing as a separate thin-main scenario,
because they no longer describe a distinct world: they are simply the subset of
Scenario A that needs no moq change. Scenario B leaves about 24,000 LOC in-tree,
dominated by the deliberate keeps of section 4 (the audio engine, Linux capture,
adaptive, sync, rooms, tests, and the render stack, the last of which leaves the
core crate for an out-of-tree `moq-video-render` crate rather than staying local
forever).
...
Merged dependency table. One vocabulary for the cut stages here, the waves of
8-upstream-plan.md, the zero-copy requirements U1-U4 (3z sec 5, drafted in 3u
sec 5), and the API decisions (3t sec 8). Identical items are merged: C2 = U1 =
...
| Wave 1 (RFC + goodwill PRs) | wave 2 (velocity gate, 8-upstream sec 5) | none |
| Wave 2 (C2, C3, C1a-c, C4) | VAAPI and V4L2 local cuts | D1/D3 RFC accepted; wave-1 review velocity |
| Wave 3 (C6, C5, C11, C12) | AV1 and Android cuts; capture-adjacent cuts | C2, C3; rav1d pin (C6); D2 decision (C5) |
...
| U4 (out-of-tree moq-video-render) | render/ leaves the core crate | U1 |
...
|---|---|---|
| render.rs + dmabuf_import + gles + gles_dmabuf + metal_import (3,463) | moq has no renderer, no import machinery, no decode-to-render GPU handoff on any platform (3z sec 2b, 2d) | move to a published out-of-tree `moq-video-render` crate over U1 public handles (U4, Option B; 3u sec 1b), not kept in-tree forever |
...
| audio_backend + AEC (2,837) | playback, mixing, declicker fades, metering, device switching, restart with backoff, and sonora AEC have zero moq counterpart (6-compare sec 3.4, 7) | stays; candidate to upstream later as a standalone playback/duplex crate |
...
| PCM codec (559) | no hang catalog codec, no interop value, essential for tests and diagnostics (3-compare sec 6) | stays forever, or trivial `Codec::Pcm` upstream if wanted |
| libcamera + libcamera_h264 (790) | the Raspberry Pi story; the pre-encoded H.264 source is unique on either side (4-compare sec 2) | keep; pre-encoded-source concept is a strong upstream candidate (C12, 3u sec 3 item 7) |
...
**R-a. Release timing (materially reduced).** The round-1 framing here warned
that the enabler stack shipped only on a dev branch whose release we did not
control, which made adoption a branch gamble. That risk is largely retired: dev
merged into main on 2026-07-21, so the whole stack is now on moq main at
`3a3e0ea8`, and adoption depends on a single ordinary release plus a version
bump rather than on a branch ever merging. What remains is only the normal wait
...
rot into visible build breaks, and unlike the round-1 hedge it can merge as soon
as the release ships, since it tracks main rather than an unmergeable dev line.
...
still possible. Before any stage here, and before any wave of 8-upstream-plan.md,
starts: re-diff `3a3e0ea8` against the then-current main or the eventual release,
...
world the 8-upstream fallback section warns about, which is why the wave-1
velocity gate and the pilot exist.

**R-d. The rav1d git-fork pin.** moq pins crates.io versions and runs
release-plz; a git dependency will not be accepted, so the AV1 upstream is gated
on publishing the pin, moving to a released rav1d, or vendoring (3-compare
sec 3; 3u sec 4). Until resolved, codec/av1/ cannot leave the tree.
...
weakens the hardware-validation offers in 8-upstream-plan.md (C1, C4, C14e):

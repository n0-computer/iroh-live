...
///
/// Uses raw V4L2 ioctls to drive the V4L2 M2M encoder, matching the sequence
/// that ffmpeg's `h264_v4l2m2m` uses. Runs in a dedicated thread with an
...
//
// Matches ffmpeg's h264_v4l2m2m sequence exactly:
//   1. Open device
...
mod raw_v4l2 {
    ...
    impl RawEncoder {
        ...
        /// This matches ffmpeg's exact startup sequence.
...
fn encoder_thread(
    ...
    let result: Result<()> = (|| {
        ...
        // Wait for first frame to start streaming (matches ffmpeg: queue first
        // OUTPUT buffer before STREAMON).

...
use iroh::Endpoint;
use iroh_live::{
    AudioTrack, Live, LiveTicket, WatchTrack, audio::AudioBackend, ffmpeg::ffmpeg_log_init,
};
...
fn main() -> Result<()> {
    ...
    let (endpoint, _broadcast, video, audio) = rt.block_on({
        ...
        async move {
            ...
            n0_error::Ok((endpoint, broadcast, video, audio))
        }
    })?;
